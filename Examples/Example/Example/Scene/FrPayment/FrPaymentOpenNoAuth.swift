...
struct FrPaymentOpenNoAuth: View {
    ...
    var body: some View {
        ...
        .navigationTitle(L10n.frPaymentOpenNoAuthTitle.localizedKey)
    ...
    func submit() {
        ...
        loading(true)
        PayByBank.frPayment.open(uniqueID: uniqueID, url: url, redirectURL: redirectURL, viewController: viewController) { result in
            loading(false)
            toast(result.string)
        }
    }
...
struct FrPaymentOpenNoAuth_Previews: PreviewProvider {
