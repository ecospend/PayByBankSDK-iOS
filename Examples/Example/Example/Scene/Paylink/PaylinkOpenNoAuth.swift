...
struct PaylinkOpenNoAuth: View {
    ...
    var body: some View {
        ...
        .navigationTitle(L10n.paylinkOpenNoAuthTitle.localizedKey)
    ...
    func submit() {
        ...
        loading(true)
        PayByBank.paylink.open(uniqueID: uniqueID, url: url, redirectURL: redirectURL, viewController: viewController) { result in
            loading(false)
            toast(result.string)
        }
    }
...
struct PaylinkOpenNoAuth_Previews: PreviewProvider {
