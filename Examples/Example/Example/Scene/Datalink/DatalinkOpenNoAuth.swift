...
struct DatalinkOpenNoAuth: View {
    ...
    var body: some View {
        ...
        .navigationTitle(L10n.datalinkOpenNoAuthTitle.localizedKey)
    ...
    func submit() {
        ...
        loading(true)
        PayByBank.datalink.open(uniqueID: uniqueID, url: url, redirectURL: redirectURL, viewController: viewController) { result in
            loading(false)
            toast(result.string)
        }
    }
...
struct DatalinkOpenNoAuth_Previews: PreviewProvider {
