...
struct VRPlinkOpenNoAuth: View {
    ...
    var body: some View {
        ...
        .navigationTitle(L10n.vrplinkOpenNoAuthTitle.localizedKey)
    ...
    func submit() {
        ...
        loading(true)
        PayByBank.vrplink.open(uniqueID: uniqueID, url: url, redirectURL: redirectURL, viewController: viewController) { result in
            loading(false)
            toast(result.string)
        }
    }
...
struct VRPlinkOpenNoAuth_Previews: PreviewProvider {
